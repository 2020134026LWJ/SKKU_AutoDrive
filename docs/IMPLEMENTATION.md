...
```
1) 차선 주행 + 장애물 회피 (차 형태, 2~3번 연속, 랜덤 위치)  ← 옆 차선으로 피했다 복귀
2) 신호등 (빨간불 정지 / 초록불 출발)
3) 트랙을 바꿔서 → 주차
```

[주의] **미션 전환 방식(수동/자동)은 규정 확인 필요.** 지금은 주차가 `parking_trigger` 토픽으로
분리돼 있어서, 사람이 쏘든 상위 노드가 쏘든 FSM은 안 건드려도 된다.

[주의] **라이다의 "장애물 = 정지" 분기 주의.** `motion_planner`는 라이다가 장애물을 잡으면
차를 **세운다.** 미션 1의 장애물은 앞 카메라(YOLO)로 보고 **피하는** 것이므로, 라이다가
같은 장애물을 보고 정지시키면 회피가 무력화된다. 라이다는 후방 장착이므로 감지 각도를
**뒤쪽으로** 잡아두면 주행 중엔 안 걸린다 (10단계 캘리브레이션).

## Phase 5 — 신규 작성 (레포에 없음)

- [x] **장애물 회피 (차선 변경)** — `obstacle_avoider_node` 신규 (2026-07-13). **미션 1.**

      원본 레포엔 회피가 없다 — 라이다가 장애물을 잡으면 **세울** 뿐이다.
      대회는 **옆 차선으로 피했다가 돌아와야** 하므로 새로 만들었다.

      **핸들을 직접 꺾지 않는다.** `lane_info_extractor`가 `cls_name='lane2'`로
      하드코딩돼 있었는데, 그 **한 줄을 파라미터로 빼는 것만으로 차선 변경이 된다.**
      회피 노드는 `target_lane`만 발행하고, 기존 경로계획이 알아서 부드러운 곡선을
      그린다. 회피가 조향값을 직접 만들면 차선 추종과 싸우게 된다.
      (제어 발행자는 여전히 motion_planner 하나 — 주차 노드와 같은 원칙)

      - YOLO 모델에 **`car` 클래스가 이미 있다** (`best_urp.pt`: car, center_line,
        lane1, lane2, left_line, right_line) → 신호등처럼 새로 짤 필요 없었다
      - FSM: FOLLOW → AVOID(옆 차선) → RETURN(복귀) → FOLLOW
      - [주의] **사라졌다고 바로 복귀하면 옆구리를 긁는다** — 눈앞에서 사라진 것과 완전히
        지나친 것은 다르다. 속도 적분으로 `pass_clearance_m`만큼 더 간 뒤 복귀
      - [주의] **옆 차선 차는 무시** — 이미 피한 그 차를 다시 보고 또 피하면 안 된다
        (bbox 중심이 내 차선 폭 안에 있을 때만 장애물로 친다)
      - [주의] **한 프레임 오검출로 차선을 바꾸지 않는다** (3프레임 연속)

      [주의] **`lane1`/`lane2` 중 어느 게 좌/우인지 모른다** — 모델 학습 때 정해진 것이라
      코드로는 알 수 없다. 실제 영상에서 눈으로 확인할 것 (docs/CALIBRATION.md 9단계).

      검증 (`python3 scripts/avoid_eval.py`, 하드웨어 0): 8/8 통과
      (회피 / 복귀 / 거리 확보 / 옆차선 무시 / 오검출 방어 / 연속 3회).
      ROS에서도 노드를 띄워 FOLLOW→AVOID(lane1)→RETURN→FOLLOW 전이 확인.
...
- [x] **신호등** — 로직 완료 (2026-07-13). `best_urp.pt`에 `traffic_light` 클래스가 없어
      ...
      **"색 비율" 방식은 못 쓴다** — 화면 위쪽의 빨강/초록 픽셀 비율로 판정하면
      ...
      [금지] **motion_planner의 빨간불 분기도 같이 고쳐야 한다** — 검출부만 바꾸고 두면,
