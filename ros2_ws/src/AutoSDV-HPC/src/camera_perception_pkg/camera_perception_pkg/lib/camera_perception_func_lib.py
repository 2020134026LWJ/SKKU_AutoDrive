...
# File: camera_perception_func_lib.cpython-310.pyc (Python 3.10)
...
def dominant_gradient(image, theta_limit):
    # 복원: 원본 .pyc(cpython-310) 바이트코드에서 재구성 (2026-07-11)
...
def draw_edge(cv_image = None, detection = None, color = None):
...
def draw_edges(detection_msg = None, cls_name = None, color = None):
...
def edge_image_postproc(cv_image = None, show_image = None):
    h = cv_image.shape[0]
    w = cv_image.shape[1]
    dst_mat = [
        [
...
def get_lane_center(cv_image, detection_height, detection_thickness, road_gradient, lane_width):
    # 복원: 원본 .pyc(cpython-310) 바이트코드에서 재구성 (2026-07-11)
    # pycdc가 None으로 잘못 대체한 3곳(cut_outliers_array/road_target_point_x/비교)을 바이트코드 기준 복구
    ...
    if (road_target_point_x == None) & (line_x_axis_pixel != None):
        ...
        if road_target_point_x > lane_width - 1:
            ...
            return road_target_point_x
        if road_target_point_x < 0:
            road_target_point_x = 0
    return road_target_point_x


def get_traffic_light_color(cv_image, bbox, hsv_ranges):
